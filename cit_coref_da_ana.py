...
#     n = g.vcount()
#     print(n, g.ecount(), g.vs.attributes(), g.es.attributes())
...
#     '''
#     progresso:
#         estou calculando o peso das arestas para que seja possivel utilizar algoritmo 
#         de filtragem de arestas na rede de co referencia
...
#     coref_edges = dd.read_csv('coref_edges_w.csv', sep='\t')
#     coref_edges.columns = ['from', 'to', 'weight']
#     g_coref = ig.Graph.TupleList(coref_edges.itertuples(index=False), directed=False, edge_attrs=['weight'])
#     g_coref.vs['name'] = g.vs['name']
#     g_coref.vs['year'] = g.vs['year']
#     g_coref.vs['refs'] = g.vs['refs']
#     g_coref.vs['title'] = g.vs['title']
...
#     g = xn.xnet2igraph('coref_net.xnet')
#     print(g.vcount(), g.ecount())
...
#         subg  = g.subgraph(vertex_seq)
#         xn.igraph2xnet(subg, 'coref_net_%s.xnet' % journal)
