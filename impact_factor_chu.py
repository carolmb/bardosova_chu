...
def get_impact(chu_journal): # citations
    ...
    dyear = 1
    papers_per_year = json.loads(open('bardo_paper_per_year_040123_nat_mat.json').read()) #'310522.json').read())
...
#         subset_papers = all_papers_citing_journal[(all_papers_citing_journal.year >= y) & (all_papers_citing_journal.year <= y+dyear)]
    ...
    out = open('impact_factor_dist_%s_010622_IFv2_231222.json' % chu_journal, 'w')
    out.write(json_str)
    out.close()
